<x-guest-layout>
    <form method="POST" action="{{ route('tasks.update', $task) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="text-center m-4">Edit the Task</div>
        <!-- Title -->
        <div>
        ...
        <div class="mt-4">
            ...
            <select class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow" name="project_id" id="project_id">
                @foreach($projects as $project)
                    <option value="{{ $project->id }}"
                        @selected(old('project_id', $project->project_id) === $project->id)>{{ $project->title }}</option>
                @endforeach
            </select>
